...
#    effbot's ElementTree module (http://effbot.org/zone/element-index.htm) is an excellent way to work with XML. 
#    It has a lot of incarnations though so distributing code that uses it right now is little bit of a pain.
#    Here is a snippet you might want to use to import one of all the available incarnations -- hopefully the users of your script will have one.
try:
        ...
        if line == 'game (':
            ...
            while line != ')':
                if line.startswith('\t'):
                    ...
                    #get category
                    try:
                        d['category'] = catver.get('Category', d['rom_name'].lower())
...
def get_xml_game_item(xml_filename):
    ...
    for event, game_element in ET.iterparse(xml_filename): #@UnusedVariable
        ...
        if game_element.tag == 'game' or game_element.tag == 'machine':
            ...
            manu = game_element.findtext('manufacturer') or ''
...
def get_catver_item(rom_name):
    #get category info
    if not os.path.exists(_catver_ini):
        return
    try:
...
def get_catver_ini(category_file):
    """load catver.ini file"""
    global catenc, catverparser
    catenc = sys.getdefaultencoding()
    if not os.path.exists(category_file):
        print _('Error: category info file: [%s] does not exist' % (category_file))
        return
    catenc = get_file_encoding(category_file)
    catverparser = ConfigParser.SafeConfigParser()
    catverparser.read(category_file)
    return catverparser
...
if __name__ == '__main__':
    ...
    #_catver_ini = os.path.join(cdir, 'catver2.ini')
